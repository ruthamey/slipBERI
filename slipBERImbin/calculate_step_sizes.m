...
% if strcmp(invert.solve_for_fault_size, 'yes') == 1 && any(m_on==0)
%     m_on_synthetic_test = [onoffidentifyer; ones(n_fault_strands_for_smoothing,1); onoffidentifyer; ones(n_InSAR_scenes,1); zeros((n_harmonics*2-1+2+2),1) ];  % Slip AND rake AND circharmparameters are off, here         (slip, alpha, rake, dip, beta, offset, circharm coeff, circharm phi, cirdharm center)
% else
%    m_on_synthetic_test = [ones(length(m_on)) ];  % never solve for circharm parameters
% end
m_on_synthetic_test = m_on;
m_on_synthetic_test(circharmparameters==1) = 0; % never solve for circharm parameters
...
if strcmp(invert.smoothing, 'VK') || strcmp(invert.smoothing, 'laplacian')|| strcmp(invert.smoothing, 'minimumnorm') == 1
    ...
    alpha2_start_number = I;
    alpha2_modelparameter_initial = m(m_identifyer== 2);
    ...
    perturbed_alpha2 = alpha2_initial;
end
...
elseif strcmp(invert.solve_for_InSAR_ramp, 'yes') == 1
    ...
    if strcmp(invert.solve_for_fault_size, 'yes') == 1
       slip_initial(onoffidentifyer==0) = [];
...
for i = 1: number_of_on_model_parameters
     ...
     % Perturb model parameters in turn (each slip patch, then alpha^2...)  
     perturbed_m = m;
     perturbed_G = G_curr;
     ...
     perturbed_offset_or_ramp = offset_or_ramp_initial;
      ...
      if strcmp(invert.smoothing, 'VK') || strcmp(invert.smoothing, 'laplacian') || strcmp(invert.smoothing, 'minimumnorm') == 1
         if strcmp(priors.alpha2_prior, 'logarithmic') == 1
                perturbed_alpha2 = 10.^perturbed_m(m_identifyer == 2);
         else
             perturbed_alpha2 = perturbed_m( m_identifyer == 2);          % either update it, or set it back to the original value, coz we do dip/rake afterwards. also have to give all of the alpha2s, even tho we've just perturbed one of them, or maybe not even any of them
         end
      end
     
      if m_identifyer(i) == 1       %solving for slip

      elseif m_identifyer(i) == 2   %solving for           already done this, coz you need to either reset alpha2 to its unperturbed value / update it, coz alpha2 is used in EVERY calculation.
 
            %disp('hi!')
            %disp(num2str(i));
            %keyboard
            % perturbed_alpha2 = perturbed_m( m_identifyer == 2);  %ALREADY DONE!    % all of the alpha2s, if there are multiple slip strands
            if strcmp(priors.alpha2flag, 'bothsame') == 1
               perturbed_alpha2modelparameter = perturbed_m(i)*ones(n_fault_strands_for_smoothing,1);
               if strcmp(priors.alpha2_prior, 'logarithmic') == 1
                    perturbed_alpha2 = 10.^perturbed_alpha2modelparameter;
               else
                    perturbed_alpha2 = perturbed_alpha2modelparameter;          % either update it, or set it back to the original value, coz we do dip/rake afterwards. also have to give all of the alpha2s, even tho we've just perturbed one of them, or maybe not even any of them
               end
            end
      ...
      elseif m_identifyer(i) == 3   %solving for rake
            ...
            perturbed_G(:,count) = cosd(perturbed_rake) * G_ss_curr(:,count) + sind(perturbed_rake) * G_ds_curr(:,count);    % just need to update the appropriate row. can use same G since we've not changed the fault
            count = count + 1;
...
%            end 
           ...
           perturbed_G = G_ss_temp*diag(cosd(rake_initial)) +  G_ds_temp*diag(sind(rake_initial));  
      ...
      elseif m_identifyer(i) == 7 || m_identifyer(i) == 8 || m_identifyer(i) == 9  || m_identifyer(i) == 10  || m_identifyer(i) == 11   % if we're changing the size of the slipping area, need to recalculate which patches are on
...
%               perturbed_slip(onoffidentifyer==0)= [];
%               perturbed_G(:, onoffidentifyer==0) = [];
%               
...
new_step_sizes(m_identifyer_master==2 & new_step_sizes < 0.001) = 0.001;
new_step_sizes(m_identifyer_master==3 & new_step_sizes < 0.001) = 0.01;
